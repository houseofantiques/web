...
(function initCoverflow(){
  const viewport = document.querySelector(".cfViewport");
  const track = document.querySelector(".cfTrack");
  ...
  function layout(){
    ...
    cards.forEach((card, i) => {
      // distance from center in circular sense
      let d = i - index;
      ...
      // Classes
      card.classList.remove("isCenter","isSide","isFar");
      if(abs === 0) card.classList.add("isCenter");
      else if(abs === 1 || abs === 2) card.classList.add("isSide");
      else card.classList.add("isFar");

      // Transform: center is big & straight
      const x = d * stepX * dirSign;
      const rY = -d * rotate * dirSign;
      const z = -abs * depth;
      const y = abs === 0 ? (-lift) : 0;
      const s = abs === 0 ? 1.05 : (abs === 1 ? .92 : (abs === 2 ? .82 : .70));
  ...
  function next(){ index = clampIndex(index + 1); layout(); }
  function prev(){ index = clampIndex(index - 1); layout(); }

  // Buttons
  document.querySelectorAll("[data-cf='next']").forEach(b => b.addEventListener("click", next));
  document.querySelectorAll("[data-cf='prev']").forEach(b => b.addEventListener("click", prev));
  ...
  function onUp(){
    ...
    // threshold
    const t = 60;
    if(moved > t) { isRTL ? next() : prev(); }      // swipe right
    else if(moved < -t) { isRTL ? prev() : next(); } // swipe left
  }
  ...
  // Optional: start from middle if you want
  // index = Math.floor(cards.length/2); layout();
})();
// Mobile menu toggle
(() => {
